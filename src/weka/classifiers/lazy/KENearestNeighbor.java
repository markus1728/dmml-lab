...
import classifier.AbstractNearestNeighbor;
 ...
 * 
 */
public class KENearestNeighbor extends AbstractClassifier implements OptionHandler {
	...
	private AbstractNearestNeighbor classifier = new NearestNeighbor();
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (knnString.length() != 0) {
			classifier.setK(Integer.parseInt(knnString));
		}
		else {
			classifier.setK(1);
		}
	...
	public void setkNearest(int kNearest) {
		classifier.setK(kNearest);
	}

	public int getkNearest() {
		return classifier.getK();
	}
